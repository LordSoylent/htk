...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        if(pindexBestHeader->nHeight > TIERED_MASTERNODES_START_BLOCK) {
	        ...
	        testValue = GetMNCollateral(pindexBestHeader->nHeight, tier);
		}
...
int64_t CMasternode::GetLastPaid()
{
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (masternodePayments.mapMasternodeBlocks.count(BlockReading->nHeight)) {
            ...
            */
            if (masternodePayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(vin.prevout, 2)) {
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 58100) return false;
    } else if (addr.GetPort() == 58100)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 HTK tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 HTK tx -> 1 confirmation
